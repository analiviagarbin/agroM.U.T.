...
import br.com.agromut.dao.AdmDAOImpl;
...
@WebServlet(name = "LoginUsuarioAdm", urlPatterns = {"/LoginUsuarioAdm"})
public class LoginUsuarioAdm extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (!loginUsuario.equals("") && !senhaUsuario.equals("")) {
                ...
                UsuarioAdm usuarioadm = (UsuarioAdm) dao.logarUsuarioAdm(loginUsuario, senhaUsuario);

                if (usuarioadm != null) {
                    HttpSession sessao = request.getSession(true);
                    idusuarioadm = usuarioadm.getIdUsuarioAdm();
                    sessao.setAttribute("idusuarioadm", idusuarioadm);
                    sessao.setAttribute("usuarioadm", usuarioadm);
                    if (usuarioadm.getStatusUsuarioAdm().equals("T")) {
                        
                        menssage = "Seja bem-vindo(a), " + usuarioadm.getNomeUsuario() + ".";
                        sessao.setAttribute("menssage", menssage);
                        request.getRequestDispatcher("usuarioAdm/index.jsp").forward(request, response);
