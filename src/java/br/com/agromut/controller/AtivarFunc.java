...
@WebServlet(name = "AtivarFunc", urlPatterns = {"/AtivarFunc"})
public class AtivarFunc extends HttpServlet {
      ...
      protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (dao.ativarfunc(usuariofunc)) {
                mensagem = "Usuário ativado!";
            } else {
                mensagem = "Problemas ao ativar usuário";
            }
            request.setAttribute("mensagem", mensagem);
            request.getRequestDispatcher("ListarUsuarioFuncT").forward(request, response);

        } catch (Exception ex) {
            System.out.println("Problemas no Servlet AtivarFunc ao enviar formulário! Erro: " + ex.getMessage());
            ex.printStackTrace();
        }
