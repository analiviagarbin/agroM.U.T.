...
@WebServlet(name = "AtivarAdm", urlPatterns = {"/AtivarAdm"})
public class AtivarAdm extends HttpServlet {
      ...
      protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (dao.ativaradm(usuarioadm)) {
                mensagem = "Usuário ativado!";
            } else {
                mensagem = "Problemas ao ativar usuário";
            }
            request.setAttribute("mensagem", mensagem);
            request.getRequestDispatcher("ListarUsuarioAdmT").forward(request, response);

        } catch (Exception ex) {
            System.out.println("Problemas no Servlet AtivarAdm ao enviar formulário! Erro: " + ex.getMessage());
            ex.printStackTrace();
        }
